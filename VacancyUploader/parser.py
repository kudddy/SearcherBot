...
from Cache.cache import regions, locality, category
...
class Parser:
    ...
    def get_jobs_api(self, num_vacs=200, is_parse=True):
        site_url = 'https://candidateapp-j237750ea.dispatcher.ru1.hana.ondemand.com/'
        self._get_input_type('api')
        jobs = []
        i = 0
        my_json = requests.get(site_url + f'api/job-requisition/v2?page={i}&size={num_vacs}').json()
